...
// -------------------------------------------------------------
// WALKABLE SKYSCRAPER BUILDER WITH GRAND LOBBY, STAIRS & INTERIORS
// -------------------------------------------------------------
function buildWalkableSkyscraper(
  ...
  const intWallMat = new THREE.MeshStandardMaterial({ color: 0xe2e8f0, roughness: 0.8 });
  ...
  // 2. Floor 2 Slab (y = 5.0) with Stairwell Opening on the right
  const floor2Slab = new THREE.Mesh(new THREE.BoxGeometry(b.w - 3.2, 0.35, b.d - 0.4), officeFloorMat);
  floor2Slab.position.set(-1.4, 5.0, 0);
  floor2Slab.receiveShadow = true;
  building.add(floor2Slab);

  // 3. Floor 3 Slab (y = 10.0) with Stairwell Opening on the left
  const floor3Slab = new THREE.Mesh(new THREE.BoxGeometry(b.w - 3.2, 0.35, b.d - 0.4), penthouseFloorMat);
  floor3Slab.position.set(1.4, 10.0, 0);
  floor3Slab.receiveShadow = true;
  building.add(floor3Slab);

  // 4. Floor 4 Slab (y = 15.0) - Tactical Armory & Server Hub
  const floor4Slab = new THREE.Mesh(new THREE.BoxGeometry(b.w - 3.2, 0.35, b.d - 0.4), officeFloorMat);
  floor4Slab.position.set(-1.4, 15.0, 0);
  floor4Slab.receiveShadow = true;
  building.add(floor4Slab);

  // 5. Floor 5 Slab (y = 20.0) - High-Altitude Observation Deck
  const floor5Slab = new THREE.Mesh(new THREE.BoxGeometry(b.w - 3.2, 0.35, b.d - 0.4), penthouseFloorMat);
  floor5Slab.position.set(1.4, 20.0, 0);
  floor5Slab.receiveShadow = true;
  building.add(floor5Slab);

  // 6. Floor 6 Slab (y = 25.0) - Executive Penthouse Suite & Sniper Nest
  const floor6Slab = new THREE.Mesh(new THREE.BoxGeometry(b.w - 3.2, 0.35, b.d - 0.4), lobbyFloorMat);
  floor6Slab.position.set(-1.4, 25.0, 0);
  floor6Slab.receiveShadow = true;
  building.add(floor6Slab);

  // 7. Roof Ceiling Slab (y = 30.0) capping the accessible 6-story interior
  const ceilingSlab = new THREE.Mesh(new THREE.BoxGeometry(b.w, 0.4, b.d), wallMat);
  ceilingSlab.position.set(0, 30.0, 0);
  building.add(ceilingSlab);

  // 8. Exterior Hollow Walls for Lower 6 Walkable Floors (y: 0 to 30.0)
  const walkableHeight = 30.0;
  // Left Exterior Wall
  const leftWall = new THREE.Mesh(new THREE.BoxGeometry(0.5, walkableHeight, b.d), wallMat);
  leftWall.position.set(-halfW + 0.25, walkableHeight / 2, 0);
  building.add(leftWall);

  // Right Exterior Wall
  const rightWall = new THREE.Mesh(new THREE.BoxGeometry(0.5, walkableHeight, b.d), wallMat);
  rightWall.position.set(halfW - 0.25, walkableHeight / 2, 0);
  building.add(rightWall);

  // Back Exterior Wall
  const backWall = new THREE.Mesh(new THREE.BoxGeometry(b.w, walkableHeight, 0.5), wallMat);
  backWall.position.set(0, walkableHeight / 2, -halfD + 0.25);
  building.add(backWall);

  // Front Left Wing
  const frontL = new THREE.Mesh(new THREE.BoxGeometry(frontWingW, walkableHeight, 0.5), wallMat);
  frontL.position.set(-halfW + frontWingW / 2, walkableHeight / 2, halfD - 0.25);
  building.add(frontL);

  // Front Right Wing
  const frontR = new THREE.Mesh(new THREE.BoxGeometry(frontWingW, walkableHeight, 0.5), wallMat);
  frontR.position.set(halfW - frontWingW / 2, walkableHeight / 2, halfD - 0.25);
  building.add(frontR);

  // Front Entrance Header (above door opening y=3.6 to y=30.0)
  const frontHeader = new THREE.Mesh(new THREE.BoxGeometry(doorW, walkableHeight - doorH, 0.5), wallMat);
  frontHeader.position.set(0, doorH + (walkableHeight - doorH) / 2, halfD - 0.25);
  ...
  // 9. Upper Tower Shaft (y: 30.0 to b.h)
  const upperHeight = Math.max(2.0, b.h - walkableHeight);
  const upperBody = new THREE.Mesh(new THREE.BoxGeometry(b.w, upperHeight, b.d), wallMat);
  upperBody.position.set(0, walkableHeight + upperHeight / 2, 0);
  building.add(upperBody);

  // Add Multi-Story Windows to the upper tower facades
  addWindowGridToBuilding(building, b.w, b.h, b.d, b.winCol, b.cols, b.floors);

  // 10. Ground Floor Grand Lobby Furnishings:
  ...
  // --- Lobby Lounge Couches & Coffee Table ---
  ...
  // --- Elevator Doors & Directory Board on Back Wall ---
  ...
  // Helper for Staircase Runs with Solid Landing Platforms immediately following each run
  const numSteps = 10;
  const stepW = 2.4;
  const stepRise = 5.0 / numSteps; // 0.50m
  const stepRun = (b.d * 0.48) / numSteps;
  const landingMat = new THREE.MeshStandardMaterial({ color: 0x334155, roughness: 0.7 });

  const addStaircaseWithLanding = (
    flightIndex: number,
    baseY: number,
    isRightSide: boolean
  ) => {
    const stairStartX = isRightSide ? halfW - 1.6 : -halfW + 1.6;
    const stairStartZ = isRightSide ? halfD - 2.8 : -halfD + 2.8;
    const dir = isRightSide ? -1 : 1;

    // 1. Build Individual Steps
    for (let s = 0; s < numSteps; s++) {
      ...
      colliders.push({
        ...
        minY: baseY,
        maxY: baseY + (s + 1) * stepRise,
        minZ: b.z + stepZ - stepRun / 2,
        maxZ: b.z + stepZ + stepRun / 2,
        name: `${b.name} Flight ${flightIndex} Step ${s + 1}`,
      });
    }

    // 2. Landing Platform immediately following the staircase run (Tactical Rest & Turn Pad)
    const landingZ = stairStartZ + dir * numSteps * stepRun + dir * 1.2;
    const landingMesh = new THREE.Mesh(new THREE.BoxGeometry(stepW + 0.4, 0.35, 2.6), landingMat);
    landingMesh.position.set(stairStartX, baseY + 5.0 - 0.18, landingZ);
    landingMesh.castShadow = true;
    landingMesh.receiveShadow = true;
    building.add(landingMesh);

    // Register Solid Landing Platform Collider
    colliders.push({
      type: 'box',
      ...
      minY: baseY + 5.0 - 0.35,
      maxY: baseY + 5.0 + 0.1,
      minZ: b.z + landingZ - 1.3,
      maxZ: b.z + landingZ + 1.3,
      name: `${b.name} Flight ${flightIndex} Landing Platform`,
    });
  };

  // Run 1: Ground (y=0) to Floor 2 (y=5.0) + Landing Platform 1
  addStaircaseWithLanding(1, 0.0, true);

  // Run 2: Floor 2 (y=5.0) to Floor 3 (y=10.0) + Landing Platform 2
  addStaircaseWithLanding(2, 5.0, false);

  // Run 3: Floor 3 (y=10.0) to Floor 4 (y=15.0) + Landing Platform 3
  addStaircaseWithLanding(3, 10.0, true);

  // Run 4: Floor 4 (y=15.0) to Floor 5 (y=20.0) + Landing Platform 4
  addStaircaseWithLanding(4, 15.0, false);

  // Run 5: Floor 5 (y=20.0) to Floor 6 (y=25.0) + Landing Platform 5
  addStaircaseWithLanding(5, 20.0, true);

  // Run 6: Floor 6 (y=25.0) to Rooftop (y=30.0) + Landing Platform 6
  addStaircaseWithLanding(6, 25.0, false);

  // 11. Furnishings for Upper Floors
  // Floor 2: Office & Security Hub (y = 5.0)
  const officeDesk1 = new THREE.Mesh(new THREE.BoxGeometry(2.6, 0.85, 1.2), deskWoodMat);
  officeDesk1.position.set(-halfW + 3.0, 5.0 + 0.42, 2.0);
  building.add(officeDesk1);
  addChest(scene, chests, chestMeshes, colliders, b.x - halfW + 3.0, 5.0 + 0.9, b.z + 2.0, 'chest');

  // Floor 3: Executive Suite & Lounge (y = 10.0)
  const boardTable = new THREE.Mesh(new THREE.BoxGeometry(3.4, 0.9, 1.8), goldTrimMat);
  boardTable.position.set(1.4, 10.0 + 0.45, 0);
  building.add(boardTable);
  addChest(scene, chests, chestMeshes, colliders, b.x + 1.4, 10.0 + 1.0, b.z, 'chest');

  // Floor 4: Tactical Armory & Server Hub (y = 15.0)
  const serverRack1 = new THREE.Mesh(new THREE.BoxGeometry(1.4, 3.4, 1.2), metalMat);
  serverRack1.position.set(0, 15.0 + 1.7, -halfD + 1.4);
  building.add(serverRack1);
  const armoryLocker = new THREE.Mesh(new THREE.BoxGeometry(2.2, 2.2, 0.8), metalMat);
  armoryLocker.position.set(-halfW + 2.5, 15.0 + 1.1, 0);
  building.add(armoryLocker);
  addChest(scene, chests, chestMeshes, colliders, b.x - halfW + 2.5, 15.0 + 1.0, b.z, 'rare_chest');

  // Floor 5: High-Altitude Observation Deck (y = 20.0)
  const obsBench = new THREE.Mesh(new THREE.BoxGeometry(3.0, 0.6, 1.0), couchMat);
  obsBench.position.set(0, 20.0 + 0.3, halfD - 2.0);
  building.add(obsBench);
  addChest(scene, chests, chestMeshes, colliders, b.x, 20.0 + 0.8, b.z + halfD - 2.0, 'chest');

  // Floor 6: Penthouse Heli-Lounge & Gold Vault (y = 25.0)
  const penthouseSofa = new THREE.Mesh(new THREE.BoxGeometry(3.2, 0.8, 1.6), couchMat);
  penthouseSofa.position.set(-halfW + 3.0, 25.0 + 0.4, 0);
  building.add(penthouseSofa);
  const vaultSafe = new THREE.Mesh(new THREE.BoxGeometry(1.6, 2.0, 1.4), goldTrimMat);
  vaultSafe.position.set(halfW - 2.4, 25.0 + 1.0, -halfD + 1.8);
  building.add(vaultSafe);
  addChest(scene, chests, chestMeshes, colliders, b.x + halfW - 2.4, 25.0 + 1.1, b.z - halfD + 1.8, 'rare_chest');

  // 12. Rooftop Architectural Props: AC Units / Water Tank / Radio Tower
  ...
  // 13. Register Solid Floor Colliders for ALL 6 Floors
  // Ground Floor Solid Collider
  colliders.push({
    type: 'box',
    minX: b.x - halfW,
    maxX: b.x + halfW,
  ...
  colliders.push({
    ...
    minZ: b.z - halfD + 0.2,
    maxZ: b.z + halfD - 0.2,
    name: `${b.name} Floor 2 Slab`,
  });

  // Floor 3 Solid Slab Collider
  colliders.push({
    type: 'box',
    minX: b.x - halfW + 3.2,
    maxX: b.x + halfW - 0.2,
    ...
    minZ: b.z - halfD + 0.2,
    maxZ: b.z + halfD - 0.2,
    name: `${b.name} Floor 3 Slab`,
  });

  // Floor 4 Solid Slab Collider
  colliders.push({
    type: 'box',
    minX: b.x - halfW + 0.2,
    maxX: b.x + halfW - 3.2,
    ...
    minZ: b.z - halfD + 0.2,
    maxZ: b.z + halfD - 0.2,
    name: `${b.name} Floor 4 Slab`,
  });

  // Floor 5 Solid Slab Collider
  colliders.push({
    type: 'box',
    minX: b.x - halfW + 3.2,
    maxX: b.x + halfW - 0.2,
    ...
    minZ: b.z - halfD + 0.2,
    maxZ: b.z + halfD - 0.2,
    name: `${b.name} Floor 5 Slab`,
  });

  // Floor 6 Solid Slab Collider
  colliders.push({
    type: 'box',
    minX: b.x - halfW + 0.2,
    maxX: b.x + halfW - 3.2,
    ...
    minZ: b.z - halfD + 0.2,
    maxZ: b.z + halfD - 0.2,
    name: `${b.name} Floor 6 Slab`,
  });

  // Skyscraper Rooftop Solid Collider
  colliders.push({
    type: 'box',
    minX: b.x - halfW,
    maxX: b.x + halfW,
  ...
  // 14. Register Perimeter Wall Colliders (Allowing free entry through front doorway)
  // Left Exterior Wall
  colliders.push({
    type: 'box',
    minX: b.x - halfW,
  ...
  addChest(
    ...
    b.h + 0.5,
    b.z - 2,
    index % 3 === 0 ? 'rare_chest' : 'chest'
  );

  // Harvestables inside skyscraper
  addHarvestable(harvestables, b.x - halfW + 3.0, 5.0, b.z - 2.0, 'wall', 'wood', 120, 20, 1.4, 0.9);
  addHarvestable(harvestables, b.x, 5.0, b.z - halfD + 1.2, 'container', 'metal', 160, 35, 1.2, 3.0);
}
